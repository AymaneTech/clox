...
Test(scanner, tokenize_all_keywords)
{
	...
	for (int i = 0; i < num_tests; i++) {
		...
		cr_assert_eq(token.type, tests[i].expected_type,
			     "Keyword test %d ('%s'): expected type %d, got %d",
			     i, tests[i].input, tests[i].expected_type,
			     token.type);

		cr_assert_eq(
			token.length, tests[i].expected_length,
...
Test(scanner, tokenize_single_char_operators)
{
	...
	for (int i = 0; i < num_tests; i++) {
		...
		cr_assert_eq(
			token.type, tests[i].expected_type,
...
Test(scanner, tokenize_two_char_operators)
{
	...
	for (int i = 0; i < num_tests; i++) {
		...
		cr_assert_eq(
			...
			"%d, got %d",
			i, tests[i].input, tests[i].expected_length,
			token.length);
	}
}
...
Test(scanner, tokenize_various_numbers)
{
	...
	for (int i = 0; i < num_tests; i++) {
		...
		cr_assert_eq(token.type, tests[i].expected_type,
			     "Number test %d ('%s'): expected type %d, got %d",
			     i, tests[i].input, tests[i].expected_type,
			     token.type);
...
{
	TokenTest tests[] = {
		{"x", TOKEN_IDENTIFIER, 1},
		{"y", TOKEN_IDENTIFIER, 1},
		{"user", TOKEN_IDENTIFIER, 4},
		{"user_age", TOKEN_IDENTIFIER, 8},
		{"myVar", TOKEN_IDENTIFIER, 5},
		{"_private", TOKEN_IDENTIFIER, 8},
